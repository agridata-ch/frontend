...
> 4 routes (4 inferred) | 0 models | 145 components | 222 lib files | 6 env vars | 20 middleware | 3 events | 25% test coverage
> **Token savings:** this file is ~17,600 tokens. Without it, AI exploration would cost ~122,600 tokens. **Saves ~105,000 tokens per conversation.**
> **Last scanned:** 2026-07-28 14:25 — re-run after significant changes
...
- `src/shared/testing/mocks/index.ts` — imported by **49** files
...
- `src/environments/environment.ts` — imported by **22** files
...
- `src/entities/api/agridata-state.service.ts` ← `src/app/analytics-service.spec.ts`, `src/app/app.component.spec.ts`, `src/app/guards/create-consent-request.guard.spec.ts`, `src/app/guards/create-consent-request.guard.ts`, `src/app/guards/home-redirect.guard.spec.ts` +80 more
...
- `src/shared/testing/mocks/index.ts` ← `src/app/app.component.spec.ts`, `src/app/error/global-error-handler.spec.ts`, `src/app/guards/login.guard.spec.ts`, `src/app/interceptors/agb-consent.interceptor.spec.ts`, `src/app/title.service.spec.ts` +44 more
