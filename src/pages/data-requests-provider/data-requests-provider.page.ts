 ...
 */
@Component({
  selector: 'app-data-requests-provider-page',
  imports: [
    FontAwesomeModule,
    I18nDirective,
    DataRequestProviderTableComponent,
    ErrorOutletComponent,
    RouterOutlet,
  ],
  templateUrl: './data-requests-provider.page.html',
})
export class DataRequestsProviderPage {
  ...
  protected newRequest = () => {
    this.router.navigate([ROUTE_PATHS.DATA_REQUESTS_PROVIDER_PATH, DATA_REQUEST_NEW_ID]).then();
  };

  protected navigateToRequest = (request?: DataRequestDto | null) => {
    if (request?.id) {
      this.router.navigate([ROUTE_PATHS.DATA_REQUESTS_PROVIDER_PATH, request.id]).then();
    }
  };
}
