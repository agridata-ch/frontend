...
describe('TableActionsComponent', () => {
  ...
  it('should call action callback and manage loading state when action button is clicked', async () => {
    ...
    // Use the fixture componentRef helper to set the input as recommended
    componentRef.setInput('actions', [action]);
    fixture.detectChanges();
    await fixture.whenStable();
    // Find action button by searching ButtonComponent instances that contain our label
    const openMenuButtons = fixture.debugElement.queryAll(By.directive(ButtonComponent));
    const actionButton = openMenuButtons.find((btn) =>
      btn.nativeElement.textContent?.includes('test.action'),
    );

    expect(actionButton).toBeDefined();

    // Trigger the action button click to invoke protected handler via template wiring
    actionButton!.triggerEventHandler('onClick', null);
