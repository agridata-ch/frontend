...
export class AgridataStateService {
  ...
  // Signals
  private readonly _userPreferences = signal<UserPreferencesDto>({});
    ...
    if (userInfo) {
      ...
      if (previousStoredUids) {
        if (Array.isArray(JSON.parse(previousStoredUids))) {
          this.addConfirmedMiratedUids(JSON.parse(previousStoredUids) as string[]);
        }
  ...
  addConfirmedMiratedUids(confirmedUids: string[]) {
    this._userPreferences.update((prefs) => {
