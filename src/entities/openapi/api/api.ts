export * from './agbRevision.service';
import { AgbRevisionService } from './agbRevision.service';
export * from './consentRequestAggregations.service';
import { ConsentRequestAggregationsService } from './consentRequestAggregations.service';
export * from './consentRequests.service';
import { ConsentRequestsService } from './consentRequests.service';
export * from './contractRevisions.service';
import { ContractRevisionsService } from './contractRevisions.service';
export * from './dataProducts.service';
import { DataProductsService } from './dataProducts.service';
export * from './dataProviders.service';
import { DataProvidersService } from './dataProviders.service';
export * from './dataRequests.service';
import { DataRequestsService } from './dataRequests.service';
export * from './dataTransfer.service';
import { DataTransferService } from './dataTransfer.service';
export * from './dataTransferV2.service';
import { DataTransferV2Service } from './dataTransferV2.service';
export * from './health.service';
import { HealthService } from './health.service';
export * from './infoResource.service';
import { InfoResourceService } from './infoResource.service';
export * from './notifications.service';
import { NotificationsService } from './notifications.service';
export * from './publicDataProducts.service';
import { PublicDataProductsService } from './publicDataProducts.service';
export * from './publicDataProviders.service';
import { PublicDataProvidersService } from './publicDataProviders.service';
export * from './publicDataSourceSystems.service';
import { PublicDataSourceSystemsService } from './publicDataSourceSystems.service';
export * from './testData.service';
import { TestDataService } from './testData.service';
export * from './uIDRegisterSearch.service';
import { UIDRegisterSearchService } from './uIDRegisterSearch.service';
export * from './users.service';
import { UsersService } from './users.service';
export const APIS = [AgbRevisionService, ConsentRequestAggregationsService, ConsentRequestsService, ContractRevisionsService, DataProductsService, DataProvidersService, DataRequestsService, DataTransferService, DataTransferV2Service, HealthService, InfoResourceService, NotificationsService, PublicDataProductsService, PublicDataProvidersService, PublicDataSourceSystemsService, TestDataService, UIDRegisterSearchService, UsersService];
